...
use MIME::Base64;
use strict;
use vars qw($opt_b $opt_d $opt_f $opt_h $opt_i $opt_k $opt_l $opt_m $opt_n $opt_p $opt_q $opt_s $opt_u $opt_v $opt_w);
use List::Util;
use Text::Wrap;
...
$opt_w = 16; # default base64 encoded lines length
...
while (<TXT>) {
  ...
  if ($start_of_cert && /^CKA_VALUE MULTILINE_OCTAL/) {
    ...
    if ( !should_output_cert(%trust_purposes_by_level) ) {
      ...
      my $encoded = "";
      my $index = 0;
      $encoded .= "0x$_," . ($index++ > $opt_w ? (($index = 0) != 0) || "\n" : " ") for unpack "(H2)*", $data;
      #$encoded =~ s/(.{1,${opt_w}})/$1\n/g;
      print CRT "/* " . $caname . " */\n";
      $caname =~ s/[^A-Za-z0-9]/_/g;
      while (exists $canames{$caname}) {
        $caname = $caname . "_";
      }
      $canames{$caname} = 1;
      print CRT "static const unsigned char certificate_" . $caname . "[] = {\n";
      # print CRT @precert if($opt_m);
      print CRT $encoded;
      print CRT "\n};\n\n";
      print CRT "#define certificate_" . $caname . "_len " . length($data) . "\n\n\n";
...
my $tl = lc $included_mozilla_trust_levels_string;
print CRT "struct ca_info_" . $tl . " {\n";
print CRT "    const unsigned char *ca_data;\n";
print CRT "    int ca_data_len;\n";
print CRT "};\n\n";

print CRT "static struct ca_info_" . $tl . " ca_certificates_" . $tl . "[] = {\n";
print CRT "{ certificate_" . $_ . ", sizeof(certificate_" . $_ . ") },\n" for keys %canames;
print CRT "};\n\n";
print CRT "static const int ca_certificates_" . $tl . "_len = " . $certnum . ";\n\n";
